...
function load_board_list() {
    ...
    if [[ $? -ne 0 || -z "$BOARD_LIST" ]]; then
        printf "[\e[31mFAIL\e[0m] Unable to retrieve boards.\n"
        exit 1
    fi
}
...
function update_config_fqbn() {
    awk -v new_fqbn="$SELECTED_FQBN" '/^[[:space:]]*FQBN[[:space:]]*==/ { sub(/==.*/, "== " new_fqbn) } { print }' "$LINUX_BUILD_CONF" > "${LINUX_BUILD_CONF}.tmp"
    if [[ $? -ne 0 ]]; then
        rm -vf "${LINUX_BUILD_CONF}.tmp"
...
function parse_config() {
    ...
    while IFS= read -r CONFIG_LINE || [[ -n "$CONFIG_LINE" ]]; do
        ...
        if [[ "$CONFIG_LINE" =~ ^([^=]+)[[:space:]]*==[[:space:]]*(.*)$ ]]; then
            ...
            elif [[ "$CONFIG_KEY" == "ADDITIONAL_OPTIONS" ]]; then
                ADDITIONAL_OPTIONS="$CONFIG_VAL"
            fi
        fi
...
function validate_options() {
    BOARD_DETAILS="$("$BINPATH" --config-file "$CONFIG_FILE" board details -b "$PARSED_FQBN")"
    if [[ $? -ne 0 ]]; then
        printf "[\e[31mFAIL\e[0m] Failed to get board details for %s\n" "$PARSED_FQBN"
        exit 1
    fi
    UNKNOWN_OPTS=""
    if [[ -n "$ADDITIONAL_OPTIONS" ]]; then
        ...
        for OPT in $ADDITIONAL_OPTIONS; do
            if ! printf '%s' "$BOARD_DETAILS" | grep -qF "$OPT"; then
                if [[ -z "$UNKNOWN_OPTS" ]]; then
                    UNKNOWN_OPTS="$OPT"
                else
    ...
    if [[ -n "$UNKNOWN_OPTS" ]]; then
        printf "the following option is unknown check config.txt\n%s\n" "$UNKNOWN_OPTS"
        exit 1
    fi
    if [[ -n "$ADDITIONAL_OPTIONS" ]]; then
        FINAL_FQBN="${PARSED_FQBN}:${ADDITIONAL_OPTIONS}"
    else
